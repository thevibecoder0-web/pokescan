...
const Scanner: React.FC<ScannerProps> = ({ onCardDetected, isScanning, setIsScanning }) => {
  ...
  const detectCardWithCV = useCallback(() => {
    ...
    try {
      ...
      for (let i = 0; i < contours.size(); ++i) {
        ...
        if (area > (c.width * c.height * 0.12)) {
          ...
          if (approx.rows === 4) {
            ...
            const width = Math.hypot(potential.tr.x - potential.tl.x, potential.tr.y - potential.tl.y);
            const height = Math.hypot(potential.bl.x - potential.tl.x, potential.bl.y - potential.tl.y);
            const ratio = width / height;

            if (Math.abs(ratio - TARGET_RATIO) < RATIO_TOLERANCE && area > maxA) {
              maxA = area;
              found = potential;
            }
          }
      ...
      if (found) { 
        ...
        // Auto-trigger AI if locked for long enough and not recently scanned
        if (!isDeepScanning && !scanResult && Date.now() - lastAIScanTime.current > 7000) {
  ...
  const runLocalOCR = useCallback(async () => {
    ...
    if (Date.now() - lastLocalOCRTime.current < 1500) return; // Limit local OCR rate
    ...
    try {
      ...
      if (ocrData) {
        ...
        if (ocrData.name && ocrData.name !== "Unknown Asset" && ocrData.number !== "???") {
          onCardDetected({ 
            id: Math.random().toString(36).substr(2,9), 
            name: ocrData.name, 
    ...
    <div className="relative w-full h-full bg-black overflow-hidden flex flex-col">
         ...
         {viewBox.w > 0 && visualCorners && (
           <svg className="w-full h-full" viewBox={`0 0 ${viewBox.w} ${viewBox.h}`} preserveAspectRatio="xMidYMid slice">
              <path 
                ...
                className="fill-cyan-400/5 stroke-[6px] stroke-cyan-400/50 transition-all duration-100" 
                style={{ transform: `translate(${jitter.x}px, ${jitter.y}px)` }}
              />
              
              <path 
                d={getQuadPath(visualCorners, TARGET_REGIONS.name)} 
                className="fill-cyan-500/20 stroke-2 stroke-white/60 animate-pulse"
              />
              <path 
              ...
              <g className="font-orbitron font-black text-[12px] fill-white">
                <text x={getQuadPoint(visualCorners, TARGET_REGIONS.name.u, TARGET_REGIONS.name.v).x} y={getQuadPoint(visualCorners, TARGET_REGIONS.name.u, TARGET_REGIONS.name.v).y - 10}>NAME_SECTOR</text>
                <text x={getQuadPoint(visualCorners, TARGET_REGIONS.number.u, TARGET_REGIONS.number.v).x} y={getQuadPoint(visualCorners, TARGET_REGIONS.number.u, TARGET_REGIONS.number.v).y - 10}>ID_SECTOR</text>
              </g>

              <g className="fill-white stroke-cyan-400 stroke-2">
                <circle cx={visualCorners.tl.x} cy={visualCorners.tl.y} r="8" />
                <circle cx={visualCorners.tr.x} cy={visualCorners.tr.y} r="8" />
                <circle cx={visualCorners.bl.x} cy={visualCorners.bl.y} r="8" />
                <circle cx={visualCorners.br.x} cy={visualCorners.br.y} r="8" />
              </g>
           </svg>
         )}
         
         <div className="absolute inset-0 border-[30px] border-slate-950/20 pointer-events-none" />
         <div className="absolute inset-0 bg-[radial-gradient(circle,rgba(34,211,238,0.05)_1px,transparent_1px)] bg-[size:40px_40px] opacity-40" />
      </div>

      {/* STATUS PANEL */}
      <div className="absolute top-10 left-1/2 -translate-x-1/2 z-50 w-full px-6 flex flex-col items-center">
        <div className={`bg-slate-900/95 backdrop-blur-3xl border-2 border-white/10 rounded-3xl px-10 py-6 shadow-2xl transition-all duration-500 ${visualCorners ? 'border-cyan-400 scale-105' : ''}`}>
           <div className="flex flex-col text-center">
              <span className="text-[10px] font-orbitron font-black text-cyan-400 tracking-[0.4em] uppercase mb-1">
                {visualCorners ? 'SIGNAL_LOCKED' : 'SEARCHING_ARCHIVES'}
              </span>
              <h2 className="text-2xl font-orbitron font-black text-white uppercase tracking-tighter">
                {detectedData?.name || (isDeepScanning ? 'AI_NEURAL_PROCESSING...' : 'AWAITING_SIGNAL')}
              </h2>
           </div>
        </div>
      </div>
      ...
      {scanResult && (
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-[70]">
           <div className="bg-slate-950/98 backdrop-blur-3xl border-[6px] border-cyan-500/50 p-16 rounded-[4rem] shadow-[0_0_150px_rgba(34,211,238,0.3)] animate-in zoom-in-95 duration-200 text-center relative overflow-hidden ring-[15px] ring-white/5">
              <div className="w-16 h-16 bg-cyan-500 rounded-full flex items-center justify-center mx-auto mb-8 shadow-xl">
              ...
              <div className="bg-cyan-500 text-slate-950 py-2 px-10 rounded-full font-orbitron font-black uppercase text-xs tracking-widest shadow-lg">ASSET_SECURED</div>
           </div>
        </div>
      )}

      {/* SCANNER CONTROLS */}
      <div className="absolute bottom-14 left-0 right-0 z-50 px-12 flex justify-between items-center">
        <button 
          onClick={() => fileInputRef.current?.click()} 
          className="pointer-events-auto backdrop-blur-3xl p-6 rounded-[2rem] border border-white/10 bg-slate-900/90 text-cyan-400 shadow-2xl transition-all active:scale-90 hover:bg-slate-800"
        >
        ...
        {torchSupported && (
          <button 
            onClick={toggleTorch} 
            className={`pointer-events-auto backdrop-blur-3xl p-6 rounded-[2rem] border border-white/10 shadow-2xl transition-all active:scale-90 ${isTorchOn ? 'bg-amber-400 text-slate-950 shadow-amber-400/40' : 'bg-slate-900/90 text-slate-400'}`}
          >
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">

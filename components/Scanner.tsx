...
const Scanner: React.FC<ScannerProps> = ({ onCardDetected, isScanning, setIsScanning }) => {
  ...
  /**
   * COMPUTER VISION: Target Acquisition
   * Frequency: Constant feed monitoring
   */
      ...
      for (let i = 0; i < contours.size(); ++i) {
        ...
        if (area > (canvas.width * canvas.height * 0.1)) {
          ...
          if (approx.rows === 4) {
            ...
            // Strict Card Ratio Check (Standard cards are 0.71)
  ...
  /**
   * STABLE SCAN LOOP: 5Hz (200ms)
   * High accuracy mode: Fewer frames, more processing time.
   */
  useEffect(() => {
    let interval: number;
    if (isScanning && cvReady && !scanResult) {
      interval = window.setInterval(async () => {
        // Step 1: Detect and fit card
        detectCardWithCV();

        // Step 2: If a stable fit is found, attempt high-fidelity extraction
        if (corners && !isProcessing) {
          ...
          if (cCtx) {
            ...
            // Precision crop with slight over-sampling for OCR clarity
            const padding = 25; 
      ...
      }, 200); // 5 times per second for maximum OCR stability
    }
  ...
  return (
    <div className="relative w-full h-full bg-black overflow-hidden flex flex-col">
      ...
      {/* HUD: High-Accuracy Perspective Mesh */}
        ...
        <div className="absolute inset-0 z-20 pointer-events-none overflow-hidden">
          <svg className="w-full h-full" viewBox={`0 0 ${viewBox.w} ${viewBox.h}`} preserveAspectRatio="xMidYMid slice">
             <path 
                ...
                className={`fill-transparent stroke-[4px] transition-all duration-200 ${detectedData?.name ? 'stroke-cyan-400 drop-shadow-[0_0_20px_rgba(34,211,238,1)]' : 'stroke-white/70 animate-pulse'}`}
             />
          ...
          >
             <div className="flex gap-2">
                <span className={`px-5 py-2.5 text-[13px] font-orbitron font-black uppercase rounded-xl shadow-2xl backdrop-blur-md ${detectedData?.name ? 'bg-cyan-400 text-black' : 'bg-slate-900/90 text-slate-500 border border-white/10'}`}>
                  {detectedData?.name || 'ANALYZING_SURFACE...'}
                </span>
             </div>
             {detectedData?.number && (
               <div className="bg-white/10 backdrop-blur-md border border-white/20 px-4 py-1.5 rounded-lg inline-block self-start">
                  <span className="text-[10px] font-orbitron font-bold text-white uppercase tracking-widest">REF: #{detectedData.number}</span>
               </div>
             )}
          </div>
        </div>
      ...
      <div className="absolute bottom-10 left-0 w-full px-10 flex justify-between items-end">
        <div className="bg-slate-950/90 backdrop-blur-3xl p-6 rounded-[2.5rem] border border-white/10 shadow-3xl min-w-[300px]">
           ...
           <div className="space-y-1">
             <div className="flex justify-between items-center">
                <span className="text-[8px] text-slate-500 uppercase font-black tracking-widest">Frame Latency:</span>
                <span className="text-[8px] text-cyan-400 font-black uppercase tracking-tight">200ms_SYNC_STABLE</span>
             </div>
             <div className="flex justify-between items-center">
                <span className="text-[8px] text-slate-500 uppercase font-black tracking-widest">OCR Core:</span>
                <span className={`text-[8px] font-black uppercase tracking-tight ${corners ? 'text-green-400' : 'text-slate-600'}`}>
                   {corners ? 'TARGET_IN_SIGHT' : 'SEEKING_LOCK'}
                </span>
             </div>
           </div>
        </div>
